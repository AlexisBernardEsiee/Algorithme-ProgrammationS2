...
static VectorStatus update_capacity(p_s_vector v) {
 ...
 p_s_vector vector_alloc(size_t n, void *data) {
    p_s_vector vector = malloc(sizeof(*p_vector));
    if (!vector) return NULL;
    vector->size = n;
    vector->capacity = vector_initial_capacity(n);
    vector->element_size = sizeof(*data);
...
#if defined(VERSION) && VERSION >= 2
VectorStatus vector_set(p_s_vector p_vector, ssize_t i, void *val) {
    if (!p_vector) {
        return VECTOR_ERROR_NULL_POINTER;
    }
    ...
    void* value;
    memcpy(value, val, p_vector->element_size);
    p_vector->data[i] = value;
    return VECTOR_SUCCESS;
}
#else
...
#endif
#if defined(VERSION) && VERSION >=3
VectorStatus vector_get(p_s_vector p_vector, ssize_t i, void *v) {
    if (p_vector == NULL || v == NULL) return VECTOR_ERROR_NULL_POINTER;

    if (i < 0 || (size_t)i >= p_vector->size) {
        return VECTOR_ERROR_OUT_OF_BOUNDS;
    }

    memcpy(v, &p_vector[i], p_vector->element_size);
    return VECTOR_SUCCESS;
